...
int TestSymmetry(SparseMatrix & A, Vector & b, Vector & xexact, TestSymmetryData & testsymmetry_data) {
 ...
 x_ncol.values = new double[262144];
 InitializeVector(y_ncol, ncol);
 y_ncol.values = new double[262144];
 InitializeVector(z_ncol, ncol);
 z_ncol.values = new double[262144];
 ...
 if (ierr) std::cout << "Error in call to dot: " << ierr << ".\n" << endl;
 ...
 if (A.geom->rank==0) std::cout << "Departure from symmetry (scaled) for SpMV abs(x'*A*y - y'*A*x) = " << testsymmetry_data.depsym_spmv << endl;
 ...
 if (A.geom->rank==0) std::cout << "Departure from symmetry (scaled) for MG abs(x'*Minv*y - y'*Minv*x) = " << testsymmetry_data.depsym_mg << endl;
 ...
 for (int i=0; i< numberOfCalls; ++i) {
   ...
   if (A.geom->rank==0) std::cout << "SpMV call [" << i << "] Residual [" << residual << "]" << endl;
 }
 DeleteVector(x_ncol);
 DeleteVector(y_ncol);
 DeleteVector(z_ncol);
