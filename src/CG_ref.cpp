...
int CG_ref(const SparseMatrix & A, CGData & data, const Vector & b, Vector & x,
  ...
  for (int k=1; k<=max_iter && normr/normr0 > tolerance; k++ ) {
    ...
    if (k == 1) {
      ...
      ComputeDotProduct_ref(nrow, r, z, rtz, t4); // rtz = r'*z
    } else {
      oldrtz = rtz;
      ComputeDotProduct_ref(nrow, r, z, rtz, t4); // rtz = r'*z
      beta = rtz/oldrtz;
      ComputeWAXPBY_ref(nrow, 1.0, z, beta, p, p); // p = beta*p + z
    }

    ComputeSPMV_ref(A, p, Ap); // Ap = A*p
    ComputeDotProduct_ref(nrow, p, Ap, pAp, t4); // alpha = p'*Ap
    alpha = rtz/pAp;
    ComputeWAXPBY_ref(nrow, 1.0, x, alpha, p, x); // x = x + alpha*p
    ComputeWAXPBY_ref(nrow, 1.0, r, -alpha, Ap, r); // r = r - alpha*Ap
    ComputeDotProduct_ref(nrow, r, r, normr, t4);
    normr = sqrt(normr);
#ifdef HPCG_DEBUG
